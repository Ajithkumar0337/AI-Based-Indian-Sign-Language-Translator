...
# Buttons (Removed "Generate Sentence" button)
Button(button_frame,
       text="Reset Session",
       font=BUTTON_FONT,
                ...
                if stabilization_buffer.count(predicted_character) > 25:
                    ...
                    if current_time - last_registered_time > registration_delay:
                        ...
                        if stable_char == ' ':
                            ...
                            word_buffer = ""
                            current_word.set("---")
                            # Update sentence with completed words
                            ...
                            word_buffer = ""
                            current_word.set("---")
                            # Save and display final sentence
                            final_sentence = sentence.strip()
                            if final_sentence:
                                save_sentence(final_sentence)
                                current_sentence.set(final_sentence)
                            else:
                                current_sentence.set("---")
                        else:
                            word_buffer += stable_char
                            current_word.set(word_buffer)
                            # Update sentence with current word progress
